...
class NeckQueue(AbstractContainer):
    ...
    def dequeue(self: "NeckQueue") -> object:
        ...
        if self.length == 0:
            raise EmptyQueueError('The neck queue is empty!')
...
class Bucket:
    ...
    def pop(self: 'Bucket') -> object:
        if not self._is_full:
            raise EmptyQueueError('Bucket is empty!')
        self._is_full = False
        return self._item
    ...
    def insert(self: 'Bucket', item: object) -> None:
        self.push(item)

    def extract(self: 'Bucket') -> object:
        return self.pop()
